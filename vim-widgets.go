...
func (rl *Shell) viCommandMode() {
	...
	if rl.keyMap == viins && !rl.cursor.AtBeginningOfLine() {
		rl.cursor.Dec()
	}
...
func (rl *Shell) viForwardChar() {
	...
	if rl.keyMap != viins && rl.cursor.Pos() < rl.line.Len()-1 {
...
func (rl *Shell) viPutAfter() {
	...
	if !rl.cursor.OnEmptyLine() && rl.cursor.Pos() < rl.line.Len() {
		rl.cursor.Inc()
	}
